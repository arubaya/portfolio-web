...
function SideNav() {
  ...
  React.useEffect(() => {
    ...
    if (isSubscribe) {
      const getUrlName = urlPath.match(/(\w+)/g);
      if (getUrlName !== null) {
    ...
    >
      <Material.Box sx={{ display: 'flex', justifyContent: 'center' }}>
        <Material.Box sx={{ position: 'absolute', height: '76px' }}>
          <Image src={imageBanner} alt="Profile banner" style={{ borderRadius: '12px 12px 0 0', width: '100%', objectFit: 'cover' }} />
        </Material.Box>
        <Material.Avatar
          alt="profile"
          sx={{
            width: '90px', height: '90px', posisition: 'relative', marginTop: '20px', border: `3px solid ${theme.palette.background.paper}`,
          }}
          children={(
      ...
      <Material.Box component="nav" sx={{ display: 'flex', flexDirection: 'column', padding: '0 20px' }}>
        {navMenuList.map((menuData) => (
          <Link href={menuData.link} key={menuData.menu}>
            <Material.ButtonBase
              onClick={() => setDrawerState(false)}
              className="navlink-container"
              sx={{
                backgroundColor: setNavState(menuData.menu, urlMenu)
                  ? theme.palette.info.main : theme.palette.background.paper,
              }}
            >
              <Material.Box>
                <Material.Typography
                  variant="button"
                  sx={{
                    color: setNavState(menuData.menu, urlMenu)
      ...
      <Material.Box sx={{
        display: 'flex', alignItems: 'center', flexDirection: 'column', margin: '50px 0 20px 0',
      }}
      >
